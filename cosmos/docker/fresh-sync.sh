...
' $NODE_HOME_DIR_LOCAL/config/config.toml > temp_config.toml && mv temp_config.toml $NODE_HOME_DIR_LOCAL/config/config.toml
 awk -v p2p_laddr="$P2P_LADDR" '
  BEGIN {OFS=FS="="}
  /^\[p2p\]$/ {p2p=1}
  p2p && $1~/^laddr/ {$2=" \"" p2p_laddr "\""; p2p=0}
  {print $0}
' $NODE_HOME_DIR_LOCAL/config/config.toml > temp_config.toml && mv temp_config.toml $NODE_HOME_DIR_LOCAL/config/config.toml
...
awk -v api_enable="$API_ENABLE" -v api_address="$API_ADDR" -v grpc_enable="$GRPC_ENABLE" -v grpc_address="$GRPC_ADDR" -v grpc_web_enable="$GRPC_WEB_ENABLE" '
  BEGIN {OFS=FS="="}
  /^\[api\]$/ {api=1; grpc=0; grpc_web=0; rosetta=0}
  /^\[grpc\]$/ {grpc=1; api=0; grpc_web=0; rosetta=0}
  /^\[grpc-web\]$/ {grpc_web=1; api=0; grpc=0; rosetta=0}
  /^\[rosetta\]$/ {rosetta=1; api=0; grpc=0; grpc_web=0}
  api && $1~/enable/ {$2=" " api_enable}
  api && $1~/address/ && !rosetta {$2=" \"" api_address "\""}
